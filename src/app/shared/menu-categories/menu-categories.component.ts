...
export class MenuCategoriesComponent {
	...
	onFilterOrStatusChange() {
		...
		if (value) {
			this.displayCategories = this.constCategories.filter((item) => item.name.toLowerCase().includes(value));
		} else {
	...
	closeMenu() {
		...
		const dropdown = (window as any).bootstrap.Dropdown.getInstance(triggerEl) || new (window as any).bootstrap.Dropdown(triggerEl);
		dropdown.hide();
	}
