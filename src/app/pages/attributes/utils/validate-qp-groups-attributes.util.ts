...
export const ValidateQPGroupsAttribute = (route: ActivatedRoute, params: any, router: Router, sortArray?: Array<string>): boolean => {
	...
	if (page !== Number(params['page']) || limit !== Number(params['limit']) || status !== params['status'] || sort !== params['sort'] || categories !== params['categories']) {
		router.navigate([], {
			relativeTo: route,
