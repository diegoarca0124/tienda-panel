...
export class ProductsBrandComponent {
		...
		this._route.params
			.pipe(
				...
				switchMap(({ brand, queryParams }) => {
					...
					if (!ValidateQPProductsBrand(this._route, queryParams, this._router, this.sortValues)) {
						return EMPTY;
					}
					...
					return forkJoin({
						products: this.initProducts$(this.filter, this.currentPage, this.limit, this.status, this.sort, this.subcategoryIds),
					});
				})
			)
			.subscribe({
	...
	initProducts() {
		this.initProducts$(this.filter, this.currentPage, this.limit, this.status, this.sort, this.subcategoryIds).pipe(takeUntil(this.destroy$)).subscribe();
	}
