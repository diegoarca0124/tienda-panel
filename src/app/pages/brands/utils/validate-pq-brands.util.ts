...
export const ValidateQPBrands = (route: ActivatedRoute, params: any, router: Router, sortArray: Array<string>, countriesArray: Array<string>): boolean => {
	...
	if (countries !== 'Todos') {
		...
		const validCountriesList = countriesList.filter((country: string) => validCountries.includes(country));

		countries = validCountriesList.length > 0 ? validCountriesList : 'Todos';
	}

	const countriesQP = Array.isArray(countries) ? countries.join(',') : countries;

	const countriesChanged = countriesQP !== (params['countries'] ?? 'Todos');

	if (page !== Number(params['page']) || limit !== Number(params['limit']) || status !== params['status'] || sort !== params['sort'] || countriesChanged) {
		router.navigate([], {
			relativeTo: route,
