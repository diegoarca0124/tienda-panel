...
@Component({
	...
	imports: [CommonModule, FormsModule, NotFoundComponent, NgbTooltipModule, IconAlertComponent],
...
export class GroupCreateProductComponent {
	...
	onSelectGroup(id: string) {
		if (!this.id) {
			...
			this.productService
				.update_group_in_product(this.id, this.productGroupEdit)
				.pipe(
	...
	init_groups() {
		...
		this.productService
			.get_groups_for_create_product(this.id, this.product.categoryId!, this.filterGroup)
			.pipe(
				takeUntil(this.destroy$),
				withMinLoadingTime(GLOBAL.MIN_LOADING_TIME),
