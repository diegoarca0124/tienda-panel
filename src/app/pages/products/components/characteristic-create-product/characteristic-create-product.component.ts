...
export class CharacteristicCreateProductComponent {
	...
	init_characteristics(id: string) {
		this.init_characteristics$(id).pipe(takeUntil(this.destroy$)).subscribe();
	}

	init_characteristics_noload(id: string) {
		this.init_characteristics_noload$(id).pipe(takeUntil(this.destroy$)).subscribe();
	}
	...
	onSearchAttribute() {
		if (this.filter) {
			...
			this.attributes = this.const_attributes
				.map((group) => ({
					...group,
					attributes: group.attributes.filter((attr: any) => regex.test(attr.name)),
				}))
	...
	onAttributeChange(group: any, attribute: any, selectedValues: any) {
		...
		const snnipet = this.const_attributes.find((item) => item.id == group.id).attributes.find((attr: any) => attr.id == attribute.id).attributeValueId;
		...
		this.productService
			...
			.subscribe({
				next: (next: { data: any; message: string }) => {
					...
					const attribute = this.attributes.find((item) => item.id === next.data.groupId)?.attributes.find((attr: any) => attr.id === next.data.attributeId);
