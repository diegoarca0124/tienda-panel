...
export class MappingCategoryComponent {
  ...
  drop(event: CdkDragDrop<any>) {
    ...
    this.categoryService.update_category_in_subcategory(subcategory.id,data)
    .pipe(
    ...
    this.categoryService
      ...
      .subscribe({
        next: (next: { data: CategoryInterface[], message: string }) => {
          ...
          this.dropListIds = this.categories.map(
            (category: CategoryInterface) => 'category-' + category.id
          );
        },
        error: (err: HttpErrorResponse) => {
          const error = err.error;
