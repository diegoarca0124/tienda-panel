export const getColorBasedOnLetter = (name: string) => {
    ...
    if (letterCode >= 97 && letterCode <= 101) {  // A - E
      return colors[0];  // bg-primary
    } else if (letterCode >= 102 && letterCode <= 106) {  // F - J
      return colors[1];  // bg-secondary
    } else if (letterCode >= 107 && letterCode <= 111) {  // K - O
      return colors[2];  // bg-success
    } else if (letterCode >= 112 && letterCode <= 116) {  // P - T
      return colors[3];  // bg-danger
    } else if (letterCode >= 117 && letterCode <= 121) {  // U - Y
